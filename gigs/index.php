...
<?php
    ...
    function print_upcoming_gigs($gig_array) {
    ...
    function print_past_gigs($gig_array) {
        foreach ($gig_array as $index => $gig) {
            $gig_date = convert_date($gig["date"]);
            echo '<div class="past-gig-container">
                    <div>
                      <span class="glyphicon glyphicon-time bullet-icon"></span><span>' . $gig_date . '</span>
                    </div>
                    <div>
                      <span class="glyphicon glyphicon-home bullet-icon"></span>
                        <a class="past-link" href="'. $gig["location_link"] .'" target="_blank">'. $gig["location_name"] .'</a>
                    </div>
                    <div>
                      <span class="glyphicon glyphicon-map-marker bullet-icon"></span>
                        <a class="past-link" href="' . $gig["map_link"] .'" target="blank">'.$gig["town_state"].'</a>
                    </div>
                  </div>';
        }
    }
...
<div class="container-fluid">
    ...
    <?php
        if (count($upcoming_gigs) > 0) {
            print_upcoming_gigs($upcoming_gigs);
        } else {
